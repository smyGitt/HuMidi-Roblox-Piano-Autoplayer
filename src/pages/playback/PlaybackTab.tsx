...
export function PlaybackTab() {
  ...
  async function chooseFile(name: string) {
    await engine.loadFile(name, name);
  ...
  return (
    <div className="playback-tab">
      ...
      <div className="playback-tab__page">
        {subTab === 0 && (
          <div className="playback-tab__file-page">
            <LoadedRow
              parts={engine.parts}
              pedalCount={engine.pedalIntervals.length}
              onEditSelection={() => setTrackSelectionOpen(true)}
        ...
        {subTab === 1 && (
          <div className="playback-tab__playback-page">
            <div className="playback-tab__two-col">
              <PerformanceCard
                config={config}
                onChange={trackedUpdateConfig}
                onReset={() => trackedUpdateConfig({ transpose: 0, pedal_style: "ai", use_velocity_accent: false })}
                midiPedalAvailable={false}
              />
              <OptionsCard
                config={config}
                onChange={trackedUpdateConfig}
                onReset={() => trackedUpdateConfig({ use_88_key_layout: false, countdown: true, debug_mode: false })}
              />
            </div>
            <TempoCard
              tempo={config.tempo}
              originalBpm={engine.originalBpm}
              onTempoChange={(v) => trackedUpdateConfig({ tempo: v })}
            />
            <PedalAiCard
              hasThresholds={engine.hasCompiledPedal}
              generateEnabled={engine.hasCompiledNotes}
              thresholdOn={config.pedal_threshold_on < 0 ? 0.5 : config.pedal_threshold_on}
              thresholdOff={config.pedal_threshold_off < 0 ? 0.5 : config.pedal_threshold_off}
              stats={engine.aiStats}
              onGenerate={engine.generatePedal}
              onThresholdChange={(on, off) => trackedUpdateConfig({ pedal_threshold_on: on, pedal_threshold_off: off })}
              onReset={() => {
                const [on, off] = engine.defaultAiThresholds ?? [0.5, 0.5];
                trackedUpdateConfig({ pedal_threshold_on: on, pedal_threshold_off: off });
              }}
            />
          </div>
        )}
        ...
        {subTab === 2 && (
          <div className="playback-tab__humanize-page">
            <HumanizeMasterRow
              humanizeAll={humanizeAll}
              simulateHands={config.simulate_hands}
              chordRoll={config.enable_chord_roll}
              onHumanizeAllChange={toggleAll}
              onSimulateHandsChange={(v) => trackedUpdateConfig({ simulate_hands: v })}
              onChordRollChange={(v) => trackedUpdateConfig({ enable_chord_roll: v })}
              onReset={() => trackedUpdateConfig({ simulate_hands: false, enable_chord_roll: false })}
            />
            <Card title="TIMING & FEEL">
              <HumRow
                name="Vary Timing"
                desc="randomize note-start jitter"
                checked={config.vary_timing}
                onCheckedChange={(v) => trackedUpdateConfig({ vary_timing: v })}
                min={0}
                max={0.1}
                value={config.timing_variance}
                suffix=" s"
                decimals={3}
                onValueChange={(v) => trackedUpdateConfig({ timing_variance: v })}
              />
              <HumRow
                name="Vary Articulation"
                desc="randomize note-hold length"
                checked={config.vary_articulation}
                onCheckedChange={(v) => trackedUpdateConfig({ vary_articulation: v })}
                min={50}
                max={100}
                value={config.articulation}
                suffix="%"
                decimals={1}
                onValueChange={(v) => trackedUpdateConfig({ articulation: v })}
              />
              <HumRow
                name="Tempo Sway"
                desc="gentle tempo rubato over time"
                checked={config.enable_tempo_sway}
                onCheckedChange={(v) => trackedUpdateConfig({ enable_tempo_sway: v })}
                min={0}
                max={0.1}
                value={config.tempo_sway_intensity}
                suffix=" s"
                decimals={3}
                onValueChange={(v) => trackedUpdateConfig({ tempo_sway_intensity: v })}
              />
              <div className="check-pair">
                <label className="check-pair__inline">
                  <input
                    type="checkbox"
                    disabled={!config.enable_tempo_sway}
                    checked={config.invert_tempo_sway}
                    onChange={(e) => trackedUpdateConfig({ invert_tempo_sway: e.target.checked })}
                  />
                  Invert Sway
                </label>
                <span className="check-pair__desc">flip the sway curve phase</span>
              </div>
            </Card>
            <Card title="HANDS & IMPERFECTION">
              <HumRow
                name="Hand Drift"
                desc="gradual left/right timing drift"
                checked={config.enable_drift_correction}
                onCheckedChange={(v) => trackedUpdateConfig({ enable_drift_correction: v })}
                min={0}
                max={100}
                value={config.drift_decay_factor}
                suffix="%"
                decimals={1}
                onValueChange={(v) => trackedUpdateConfig({ drift_decay_factor: v })}
              />
              <HumRow
                name="Mistake Chance"
                desc="occasional dropped or mistimed notes"
                checked={config.enable_mistakes}
                onCheckedChange={(v) => trackedUpdateConfig({ enable_mistakes: v })}
                min={0}
                max={10}
                value={config.mistake_chance}
                suffix="%"
                decimals={1}
                onValueChange={(v) => trackedUpdateConfig({ mistake_chance: v })}
              />
            </Card>
          </div>
        )}
